...
const startFunc = ({ inConfig, inClassName }) => {
    ...
    if (inConfig.isWrapper && Array.isArray(inConfig?.elements)) {
        const mainDiv = document.createElement("div");
        // console.log("jjjjjjjjjj : ", mainDiv);
        // mainDiv.className = inConfig.theme ? themes[inConfig.theme] : "flex flex-col w-full gap-2";
        mainDiv.className = "flex flex-col w-full gap-2";

        inConfig?.elements.forEach(element => {
            const div = document.createElement("ks-wrapper-base");

            div.config = element;
            div.setAttribute("ks-theme", "inline-30-70");
